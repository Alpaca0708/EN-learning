...
export default async function getSeriesData(slug) {
    ...
    // The `params` object contains the dynamic parts of the URL, in this case, the 
    //   `slug`
    export default async function SingleSeriesPage({ params }) {
      const { slug } = params;
      const series = await getSeriesData(slug);
    
      // If the series is not found, you can render a custom message or a not-found 
    //   component.
      if (!series) {
        return (
          <Layout>
      ...
      return (
        <Layout>
          <div className="px-4 sm:px-10 md:px-20 lg:px-40 flex flex-col items-center 
      py-5 text-white">
      ...
      object-cover w-full h-full rounded-lg shadow-lg"
              ...
              {/* Seasons Section */}
              <div className="mt-12">
                <h2 className="text-3xl font-bold border-b-2 border-gray-700 pb-2 
      mb-4">Seasons</h2>
      ...
      duration-200 rounded-lg p-4 text-center"
                    >
                      <span className="text-xl font-semibold">Season {season.
      seasonNumber}</span>
                    </Link>
                  ))}
                </div>
                {(!series.seasons || series.seasons.length === 0) && (
                  <p className="text-gray-400">No seasons available for this series
      yet.</p>
                )}
              </div>
            </div>
